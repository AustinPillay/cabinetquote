...
def build_header(job, S):
    ...
    t = Table(header_data, colWidths=[W * 0.55, W * 0.35])
    ...
    c = Table(client_data, colWidths=[W * 0.55, W * 0.35])
...
def build_cabinet_table(cabinets, S):
    ...
    for i, cab in enumerate(cabinets):
        ...
        tt = Table(title_data, colWidths=[W * 0.45, W * 0.45])
        ...
        rt = Table(row_data, colWidths=[W * 0.55, W * 0.35])
...
def build_totals_table(totals, transport, S):
    ...
    t = Table(row_data, colWidths=[W * 0.55, W * 0.35])
    t.setStyle(TableStyle([
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python generatePDF.py <job.json> <output.pdf> [type]")
        print("  type: quote | cutlist | hardware | all (default: all)")
        sys.exit(1)

    json_path  = sys.argv[1]
    pdf_path   = sys.argv[2]
    pdf_type   = sys.argv[3] if len(sys.argv) > 3 else "all"
